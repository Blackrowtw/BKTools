...
public class Configs implements IConfigHandler {
        ...
        // ── 快捷鍵設定 ───────────────────────────────────────
        // 格式："KEY" 或 "MODIFIER+KEY"，例如 "B,C" 代表 B 和 C 同按
        // apply() 的字串是 translation key 的前綴
        // malilib 會自動組合成：{前綴}.name.{config名稱} 和 {前綴}.comment.{config名稱}
        public static final ConfigHotkey OPEN_GUI_CONFIGS = new ConfigHotkey("openGuiConfigs", "B,C",
                        KeybindSettings.PRESS_ALLOWEXTRA)
                        .apply(Reference.MOD_ID + ".config");
